...
public class Eventsaver {
	...
	protected static ArraylistofArraylist replicatoday = new ArraylistofArraylist();
	...
	public static void deleteexpiredevents(LocalDate newdate , LocalDate olddate) {
		...
		if(x == 1) {
			//Μετακίνηση ολων των στοιχειω του 14days μια θεση παρακατω και μεταφορα των στοιχειων today στον 14days[0]
			for(int i = 13 ; i > 0 ; i--) {
			...
			for(int i = 0 ; i < Eventsaver.today.eventlist.size() ; i++ ) {
				for(int j = 0; j < Eventsaver.today.eventlist.get(i).size(); j++) {
						...
						replicatoday.eventlist.get(i).add(today.eventlist.get(i).get(j));
						
				}
			}
			fourteendays[0] = replicatoday;
